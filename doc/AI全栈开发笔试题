...
调用 Gemini AI Studio 免费 API，基于 YouTube 字幕生成一篇中文视频对话内容文章。文章需有清晰排版，并在网页中渲染为HTML。
主文章生成必须使用流式输出：生成一点输出一点，并实时展示在网页上。
遇到 youtube 验证码怎么解决 ？
如果YouTube提取字幕遇到验证码，可以考虑用 webshare.io 的代理（免费账号有10个代理，无需信用卡，用谷歌登录即可）。
...
1. 页面还需支持输入一段自然语言生成要求（使用时可选）。生成内容应能体现用户要求中的约束（不一定都覆盖，但不会超过这个范围）：
  ...
  - 约束条件
2. 5W1H总结
  - 主文章生成结果需要按章节组织。每个章节需提供「5W1H 总结」能力：用户点击章节标题旁[5W1H]按钮后，页面展示该章节的 Who / What / When / Where / Why / How 总结。
...
生成的对话排版 对话安德森：AI革命的万亿美金之问
比如，上文"智能经济：收入爆发与成本塌陷"章节，这一章节的5W1H生成结果示例如下（展示形式自由）
Who
Mark
What

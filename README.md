...
| `DEEPSEEK_API_KEY` | DeepSeek API Key（可选，启用 ?provider=deepseek 路由） |
...
通过 URL 查询参数 `?provider=` 显式切换，前端提供下拉菜单：

- **Gemini 默认**：`/api/generate` 无参数时使用 Gemini 2.5 Flash（`generateContentStream`）
- **DeepSeek**：`/api/generate?provider=deepseek` 使用 DeepSeek V4 Flash（OpenAI 兼容 API，1M 上下文，无 `max_tokens` 限制）
- `?provider=unknown` 自动回退到 Gemini
- 两种 provider 共用 `prompts.ts` 中的双格式模板（纯文本 + chat messages）

> 本地开发推荐 DeepSeek——WSL/workerd 可直接访问，无地域限制，无日配额。
...
1. `Innertube.create()` — 创建会话，传入自定义 `fetch` 函数（设置 `AbortSignal.timeout(5000ms)` 防止在受限环境中无限挂起）。设置 `retrieve_player: false` 跳过 YouTube 播放器脚本——只提取字幕，不需要视频播放签名解密，避免 `[YOUTUBEJS][Player]: Failed to extract signature decipher algorithm` 错误
...
- **句子级批量推送**：Server 端累积 token 直到命中句子结束符（`。！？\n`）或达到 40 字符阈值，再一次性 flush 为 SSE 消息。将单篇文章的 SSE 消息数从 ~1000 条降到 ~50-100 条，大幅减少协议开销
- **换行处理**：SSE 协议用 `\n\n` 作为消息分隔符。DeepSeek 生成的内容中包含 `\n`（段落换行），若直接放入 SSE payload 会破坏消息边界。Server 端将内容按 `\n` 分割，用空 payload（`data: \n\n`）作为换行标记；前端将空 payload 转换为 `\n` 追加到行缓冲
- **入场动画**：新段落/chapter 添加 `fadeInUp` CSS 动画（opacity 0→1 + translateY 12px→0，0.4s ease-out），生成期间文末显示蓝色闪烁光标，完成后自动移除
...
| SSE 传输 → 句子级批量 + 行级增量解析 | 不处理嵌套 Markdown（列表/代码块），只解析 #/## /段落 | 批量推送降 SSE 消息量 90%+，行级解析 O(n)，覆盖 95% 场景 |
| AI provider → `?provider=` query 参数路由 | 每个请求独立选择 provider | 前端下拉菜单显式切换，默认 Gemini，`?provider=deepseek` 激活 DeepSeek |
...
| SSE 批量推送 → 句子级延迟 | 牺牲逐 token 的实时感 | 协议开销降低 90%+，配合 fadeInUp 动画体验更好 |
| youtubei.js → retrieve_player: false | 无法获取视频流 URL（本就不需要） | 避免 YouTube 更新播放器代码时报 signature decipher 错误 |
...
- **AI**: Gemini 2.5 Flash + DeepSeek V4 Flash（OpenAI 兼容，1M 上下文）

...
1. **YouTube 实时字幕提取不稳定。** Cloudflare Workers 出口 IP 被 YouTube 严格反爬：Innertube WEB 客户端返回空字幕、IOS/ANDROID 客户端返回 400、TCP 代理 TLS 握手被 SNI 阻断。当前仅 `youtubei.js` 的 `caption_tracks` XML 回退在某些视频上成功，其余场景自动降级至硬编码 demo 字幕。这是 Cloudflare 平台层面的限制，非代码缺陷——换成非 CF IP 的服务（如 VPS）可正常提取。详见上节「YouTube 字幕获取：验证码与反爬」。

2. **CPU 使用率仍需持续优化。** 当前透传架构已将 Worker CPU 降至 ~0.8ms（字幕提取 + `fetch`/`return`），在免费计划 10ms 限制内运行稳定。但 SSE 解析（`TextDecoder`、`indexOf`、正则匹配）仍全部发生在前端浏览器中，受设备性能影响。后续优化方向包括：前端 SSE 解析使用更高效的流处理方案，或评估将 AI 调用完全移至外部服务以避免 Worker 参与任何网络中转。

...
输入 YouTube 链接，AI 提取字幕并生成结构化中文文章，支持流式渲染和章节级 5W1H 摘要。PC 端侧栏可折叠，移动端目录抽屉式展开，章节默认折叠可按需阅读。
...
│ { youtubeUrl, rule? }            │ ←────────── │ tee SSE → KV save (stream)   │
│                                  │  SSE + sid  │                              │
│ JSON.parse → feedContent()       │             │ POST /api/5w1h               │
│ → marked.parse() → DOM           │ ──────────→ │ KV read → AI fetch           │
│                                  │ ←────────── │ parse nested JSON → flat obj │
│ rAF throttle + TOC live update   │    JSON     │                              │
└──────────────────────────────────┘             └──────────────────────────────┘
```

AI API Key 存于 Worker secrets，前端不暴露。Worker 对 AI 响应采用单 reader 流包装：透传原始字节给客户端的同时解析 SSE 累积文章全文，流结束后异步存入 KV（24 小时 TTL），供 5W1H 摘要使用。5W1H 调用由 Worker 端解析 AI 嵌套 JSON 响应，返回扁平 `{who, what, when, where, why, how}` 对象。
...
- `tailwind.css` → Tailwind CLI 编译 → `public/app.css`
...
- 目录（TOC）在流式过程中实时更新，流完成后注册滚动追踪
- 侧栏在 PC 端可折叠，带动画过渡，生成时自动收起
- 章节默认折叠，点击展开；5W1H 摘要置于章节内容开头

## 5W1H 摘要

每个章节旁注入 `[5W1H]` 按钮（Sparkles 图标 + 渐变背景），点击后由 Worker 读取 KV 中缓存的文章全文，调用 AI API 并**在服务端解析嵌套响应**，返回扁平 JSON：
...
├── frontend/
│   ├── app.js            # SSE 解析 + marked 渲染 + rAF 节流 + TOC + 侧栏
│   ├── tailwind.css      # Tailwind 源文件（组件 + 工具类）
│   ├── locale.js         # 中/英 i18n 字符串
│   └── index.html        # 入口页面
├── fallback-subtitles/
...
1. **YouTube 实时字幕提取不稳定。** Cloudflare Workers 出口 IP 被 YouTube 反爬，多数方案失败，仅 `caption_tracks` 部分可用，否则降级至 demo。降级时前端显示 ⚠️ 警告横幅。

2. **CPU 使用率。** 透传 + tee 架构下 Worker CPU 保持在免费计划 10ms 限制内。tee 模块通过单 reader `ReadableStream` 同时做透传和 SSE 解析，避免 `tee()` 的双 reader 开销。

3. **Session 过期。** 文章 KV 缓存 TTL 24 小时。过期后 5W1H 请求返回 404，需重新生成文章。
